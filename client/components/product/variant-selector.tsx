...
export function VariantSelector({ characteristics }: { characteristics: Characteristic[] }) {
  const options = [
    {
      id: 1,
      name: 'Characteristic',
      values: characteristics.map((c) => c.title)
    }
  ];
  ...
  const optimizedVariants: OptimizedVariant[] = characteristics.map((variant) => {
    ...
    const name = 'characteristic'.toLowerCase();
