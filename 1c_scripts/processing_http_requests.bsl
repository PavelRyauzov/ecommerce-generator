...
Функция ПолучитьДанныеНоменклатуры() Экспорт
	...
	// Заказы клиентов
	ЗаказыКлиентов = ПолучитьЗаказыКлиентов();
	...
	КорневойОбъект.Вставить("orders", ЗаказыКлиентов);
...
Функция ПолучитьНоменклатуруОпределенныхВидов(НаименованияВидовНоменклатуры)
	...
	Для Каждого НаименованиеВидаНоменклатуры из НаименованияВидовНоменклатуры Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Картинки = ПолучитьКартинкиТовараПоСсылке(Выборка.Ссылка);
...
Функция ПолучитьКартинкиТовараПоСсылке(ТоварСсылка)
	Запрос = Новый Запрос;
	...
	Пока Выборка.Следующий() Цикл
		...
		Картинка.Вставить("base64", ?(ЗначениеЗаполнено(ФайлКартинки) И ФайлКартинки <> Null, ПолучитьКартинкуПоСсылке(ФайлКартинки), ""));
...
&НаСервере
Функция ПолучитьКартинкуПоСсылке(СсылкаНаФайл)
...
Функция ПолучитьЗаказыКлиентов()
	Запрос = Новый Запрос;
	...
	|ГДЕ
 	| ЗаказКлиента.Дата > ДАТАВРЕМЯ(2023, 4, 1, 0, 0, 0)";

	Выборка = Запрос.Выполнить().Выбрать();

	ЗаказыКлиентов = Новый Массив;
	Пока Выборка.Следующий()Цикл
		ЗаказКлиента = Новый Структура;
		ЗаказКлиента.Вставить("external_id", "" + Выборка.Ссылка.УникальныйИдентификатор());
		ЗаказКлиента.Вставить("date", Выборка.Дата);
		ЗаказКлиента.Вставить("status", "" + Выборка.Статус);

		АдресДоставки = Выборка.АдресДоставки;
		ЗаказКлиента.Вставить("delivery_address", ?(ЗначениеЗаполнено(АдресДоставки) И АдресДоставки <> Null, АдресДоставки, Неопределено));

		ЗаказКлиента.Вставить("order_items", ПолучитьТоварыИзЗаказаКлиентаПоСсылке(Выборка.Ссылка));
		ЗаказыКлиентов.Добавить(ЗаказКлиента);
	КонецЦикла;

	Возврат ЗаказыКлиентов;
КонецФункции

Функция ПолучитьТоварыИзЗаказаКлиентаПоСсылке(СсылкаНаЗаказКлиента)
	Запрос = Новый Запрос;
	...
	ТоварыИзЗаказа = Новый Массив;
	Пока Выборка.Следующий()Цикл
		ТоварИзЗаказа = Новый Структура;
		ТоварИзЗаказа.Вставить("external_id", "" + Выборка.СсылкаЗаказКлиента.УникальныйИдентификатор());
		ТоварИзЗаказа.Вставить("external_product_id", "" + Выборка.Номенклатура.УникальныйИдентификатор());

		Характеристика = Выборка.Характеристика;
		ТоварИзЗаказа.Вставить("external_product_characteristic_id", ?(ЗначениеЗаполнено(Характеристика) И Характеристика <> Null, "" + Характеристика.УникальныйИдентификатор(), Неопределено));

		ТоварИзЗаказа.Вставить("external_warehouse_id", "" + Выборка.Склад.УникальныйИдентификатор());
		ТоварИзЗаказа.Вставить("quantity", Выборка.Количество);
		ТоварИзЗаказа.Вставить("price", Выборка.Цена);
		ТоварыИзЗаказа.Добавить(ТоварИзЗаказа);
	КонецЦикла;

	Возврат ТоварыИзЗаказа;
КонецФункции
